<script lang="ts">
	import { FileText, FileJson, Download, Info } from 'lucide-svelte';
	...
	// Tooltip content
	let delimiterTooltip =
		"Choose the delimiter for your CSV/TSV file. 'Auto-detect' attempts to identify it automatically.";
	...
	let llmTooltip =
		'Choose a Language Model (LLM) for data analysis. Different models offer varying levels of detail and insights.';
			...
			Papa.parse(newFile, {
				...
				complete: function (results) {
					...
					if (autoDetectColumns) {
						const cpgGuesses = availableColumns.filter((col) => /cpg|probe|cg\d+/i.test(col));
	...
	async function startProcessing() {
		...
		try {
			...
			await new Promise((resolve, reject) => {
				// @ts-expect-error - PapaParse type definitions are inconsistent
			...
			heartbeatInterval = setInterval(async () => {
				...
				if (!requestId || !isProcessing) {
					console.log(
						`Stopping heartbeat for request ${requestId} - processing complete or cancelled. State:`,
						{
							requestId,
							isProcessing,
							currentPhase,
				...
				try {
					...
					console.error('Error sending client heartbeat');
				}
			}, 5000);
	...
	async function handleDrop(event: DragEvent, type: 'csv' | 'json') {
		...
		if (type === 'csv') {
			if (
				droppedFile &&
				(droppedFile.type.startsWith('text/') || /\.(csv|tsv|txt)$/i.test(droppedFile.name))
			) {
	...
	function getAverageMatchStrength(finding: any): number {
		...
		return (
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			regions.reduce((sum: number, region: any) => sum + (region.matchStrength || 0), 0) /
			regions.length
		);
	}
	...
	function insertSorted(newFinding: any, currentFindings: any[]): any[] {
		...
		const insertIndex = currentFindings.findIndex(
			(finding) => getAverageMatchStrength(finding) < newStrength
		);
	...
	function slideAndFade(node: Element, { delay = 0, duration = 300 }) {
		return {
			delay,
			duration,
			css: (t: number) => {
...
<main class="analyze-container {isAnyCardExpanded ? 'sm:expanded' : ''}">
	<div
		class="insight-card p-3 mt-8 w-[300px] h-fit transition-all duration-300 ease-in-out"
		style="position: sticky; top: 2rem; {isAnyCardExpanded
			? 'sm:transform: translateX(-120%);'
			: ''}"
	>
		<div class="flex flex-col gap-3">
			...
			<div class="p-6 rounded-lg border border-gray-700 bg-aeon-surface-1/50">
				...
				{#if !isProcessing && !isComplete}
					<div class="mb-1">
						<p class="text-blue-400 text-sm">
							<a
								href="https://www.ncbi.nlm.nih.gov/gds/?term=(%22methylation%22%5BMeSH+Terms%5D+OR+methylation%5BAll+Fields%5D)+AND+profiling%5BAll+Fields%5D+AND+(%22humans%22%5BMeSH+Terms%5D+OR+%22Homo+sapiens%22%5BOrganism%5D+OR+homo+sapiens%5BAll+Fields%5D)+AND+peripheral%5BAll+Fields%5D+AND+mononuclear%5BAll+Fields%5D+AND+(%22blood%22%5BSubheading%5D+OR+%22blood%22%5BMeSH+Terms%5D+OR+blood%5BAll+Fields%5D)"
								target="_blank"
								rel="noopener noreferrer"
								>Find methylation data matrix<br />(normalized beta values, Illumina)</a
							>
						</p>
					</div>
				...
				<div>
					<p class="text-gray-300 mb-2 text-sm">
						<a
							href="https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE147740"
							target="_blank"
							rel="noopener noreferrer"
							class="text-blue-400">About population reference</a
						>
					</p>
				</div>
						...
						{#if isLoadingColumns}
							<div class="flex flex-col items-center py-4">
								...
								<div class="text-center">
									...
									<span class="text-xs text-gray-400 mt-0.5 block"> Detecting columns </span>
								</div>
							</div>
						{:else}
							<div class="relative">
							...
							<button
								...
								disabled={!csvConfig.cpgColumn ||
									!csvConfig.sampleColumn ||
									!csvConfig.selected_llm}
							>
				...
				<!-- CSV Upload Zone -->
				<label
					class="file-drop-zone flex flex-col items-center gap-2 px-4 py-6 rounded-lg border-2 border-dashed transition-all duration-200 {isDragging.csv
						? 'border-aeon-primary bg-aeon-primary/10'
						: 'border-gray-700 bg-aeon-surface-1'} {!isDragging.csv
						? 'hover:border-aeon-primary hover:transform hover:translate-y-[-1px] hover:shadow-lg'
						: ''} cursor-pointer"
				...
				>
					...
					<div class="text-center">
						...
						<span class="text-xs text-gray-400 mt-0.5 block"> or click to browse </span>
					</div>
				</label>
			</div>
			...
			<div class="p-4 rounded-lg border border-gray-700 bg-aeon-surface-1/50">
				...
				<!-- JSON Upload Zone -->
				<label
					class="file-drop-zone flex flex-col items-center gap-2 px-4 py-6 rounded-lg border-2 border-dashed transition-all duration-200 {isDragging.json
						? 'border-aeon-primary bg-aeon-primary/10'
						: 'border-gray-700 bg-aeon-surface-1'} {!isDragging.json
						? 'hover:border-aeon-primary hover:transform hover:translate-y-[-1px] hover:shadow-lg'
						: ''} cursor-pointer"
		...
		<div class="findings-container">
			...
			<div class="findings-list">
				{#each uniqueFindings as finding, index ((finding.id ?? finding.provenance?.associated_regions?.[0]?.matched_gene) + '-' + index)}
					<div class="finding-item" transition:slideAndFade={{ duration: 300 }}>
						<InsightCard
							{finding}
	...
	{#if showKeepOpenAlert}
		<div class="fixed bottom-4 right-4 max-w-sm" transition:slide>
			<div
				class="bg-aeon-surface-1/95 backdrop-blur-md border-l-4 border border-l-yellow-500 border-aeon-primary/20 rounded-lg p-4 shadow-lg"
			>
				<div class="flex gap-3">
					...
					<div>
						...
						<p class="mt-1 text-sm text-gray-300">
							Please don't close or hide this tab during analysis. Hidden or background tabs will
							lose connection and interrupt the process.
						</p>
...
<style>
	...
	.findings-header::after {
		...
		right: 0;
		height: 1px;
		background: linear-gradient(90deg, transparent, rgba(76, 201, 240, 0.2) 50%, transparent);
	}
