<script lang="ts">
	...
	$: currentConfig = currentSectionId
		? sectionGridConfigs[currentSectionId]
		: sectionGridConfigs['hero-section'];
...
<div class="grid-container">
	<div
      ...
      --base-color: {currentConfig.baseColor.replace(
			'%%INTENSITY%%',
			gridBaseIntensity.toString()
		)};
      --accent-color: {currentConfig.accentColor.replace(
			'%%INTENSITY%%',
			gridAccentIntensity.toString()
		)};
...
<style>
	...
	.grid-pattern {
		...
		transform: perspective(1000px) rotateX(var(--rotation)) translateZ(0)
	...
	.grid-fade {
		position: absolute;
		inset: 0;
		background: linear-gradient(180deg, transparent 0%, var(--aeon-deep-space) 75%);
		opacity: 0.9;
	}
</style>
