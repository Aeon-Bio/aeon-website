...
<div class="partnership-form">
	{#if !isSubmitted}
		<div class="form-progress" role="progressbar" aria-valuemin="1" aria-valuemax="2" aria-valuenow={currentStep} aria-label="Form progress">
		...
		<form on:submit|preventDefault={currentStep === 2 ? submitForm : nextStep}>
			...
			{#if currentStep === 2}
				<div class="form-step" transition:fade={{ duration: 200 }}>
					...
					<div class="form-actions">
						<button type="button" class="btn-secondary" on:click={prevStep} aria-label="Go back to previous step">
							<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
								<path d="m15 18-6-6 6-6"/>
							</svg>
							Back
						</button>
						<button type="submit" class="btn-primary" disabled={isSubmitting} aria-describedby="submit-status">
							{#if isSubmitting}
	...
	{:else}
		<div class="success-state" transition:fade={{ duration: 300 }}>
			...
			<p>We've received your partnership inquiry and will be in touch within 48 hours to discuss how we can collaborate.</p>
			<p class="next-steps">In the meantime, feel free to explore our research publications or schedule a demo call.</p>
