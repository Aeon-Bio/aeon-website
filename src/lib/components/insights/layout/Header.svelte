<script lang="ts">
	...
	$: geneList = [
		...new Set(
			finding.provenance.associated_regions.map((region) => region.matched_gene || region.gene_name)
		)
	];
</script>

<div class="p-6 pb-3 cursor-pointer" on:click={handleClick}>
	<div class="flex items-start justify-between">
		<div class="space-y-2">
			<div class="flex items-center gap-2">
				...
				<span
					class="px-2 py-1 rounded-full text-xs {finding.pattern.direction === 'hypo'
		...
		<ChevronDown
			class={`h-6 w-6 text-aeon-primary transition-transform ${expanded ? 'rotate-180' : ''}`}
		/>
	</div>
</div>
