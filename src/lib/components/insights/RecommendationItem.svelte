			...
			{#if recommendation.specificActions?.length}
				<div class="space-y-3">
					...
					<div class="grid gap-2">
						{#each recommendation.specificActions as action (action)}
							<div
								class="flex items-start gap-3 bg-aeon-surface-1/50 rounded-lg hover:bg-aeon-surface-1 transition-colors"
							>
			...
			<div class="grid gap-6 grid-cols-1 md:grid-cols-2">
				...
				{#if recommendation.monitoringMetrics?.length}
					<div class="space-y-3">
						...
						<div class="grid gap-2">
							{#each recommendation.monitoringMetrics as metric (metric.metric)}
								<div
									class="bg-aeon-surface-1/50 rounded-lg hover:bg-aeon-surface-1 transition-colors"
								>
				...
				{#if recommendation.supportingEvidence?.length}
					<div class="space-y-3">
						...
						<div class="space-y-2">
							{#each recommendation.supportingEvidence as evidence (evidence)}
								<div
									class="bg-aeon-surface-1/50 rounded-lg hover:bg-aeon-surface-1 transition-colors"
								>
				...
				<div class="pl-2 space-y-3">
					<div class="bg-aeon-surface-1 p-4 rounded-lg border-l-4 border-yellow-500">
						...
						<div class="space-y-2 ml-10">
							{#each Array.isArray(recommendation.contraindications) ? recommendation.contraindications : [recommendation.contraindications] as contraindication (contraindication)}
		...
		{#if showStrengthInfo}
			...
			>
				{strengthDescriptions[
					recommendation.strength.toLowerCase() as keyof typeof strengthDescriptions
				]}
			</div>
		{/if}
	</div>
