<script lang="ts">
	...
	$: studyIds = [
		...new Set(finding.provenance.associated_regions.flatMap((region) => region.studyIds || []))
	];
</script>
...
<div class="space-y-6">
	...
	{#if studyIds.length}
		<div class="bg-aeon-surface-1 p-4 rounded-lg">
			...
			<div class="space-y-2">
				{#each studyIds as studyId (studyId)}
					<a
				...
				{#if finding.provenance.studyContext.studyMetrics[selectedStudyIndex]?.studyPopulationSummary?.length}
					<div>
						...
						<div class="grid gap-3">
							{#each finding.provenance.studyContext.studyMetrics[selectedStudyIndex].studyPopulationSummary as group (group.groupName)}
	...
	<div class="bg-aeon-surface-1 p-4 rounded-lg">
		...
		<div class="space-y-4">
			...
			<div>
				...
				<div class="space-y-2">
					{#each finding.evidence.mechanisticInsights as insight (insight)}
			...
			{#if finding.evidence.functionalImpact?.length}
				<div>
					...
					<div class="space-y-2">
						{#each finding.evidence.functionalImpact as impact (impact)}
							<div class="flex items-start gap-2">
								<ChevronsRight class="h-4 w-4 mt-1 text-aeon-primary flex-shrink-0" />
			...
			<div class="space-y-4">
				...
				{#if finding.provenance.associated_regions[selectedRegionIndex].modifyingFactors?.length}
					<div>
						...
						<div class="space-y-3">
							{#each finding.provenance.associated_regions[selectedRegionIndex].modifyingFactors as factor (factor.factor)}
								<div class="bg-aeon-surface-0 p-3 rounded-lg">
