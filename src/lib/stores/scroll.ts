...
export const scrollTracker = readable(initialState.position, (set) => {
  ...
  const updateScrollPosition = () => {
    ...
    const normalizedProgress = documentHeight > 0 ? Math.max(0, Math.min(1, currentY / documentHeight)) : 0;
    ...
    const updateSections = () => {
      const sections: SectionInfo[] = sectionIds.map(id => {
        ...
        if (bounds && element) {
          ...
          inView = elementBottom > viewportTop - windowHeight * 0.2 && 
      ...
      scrollState.update(state => {
        ...
        return {
          ...
          sections,
          currentSection,
          previousSection: currentSection !== previousSection ? previousSection : state.previousSection,
          transitionProgress
        };
      });
    };
...
export const scrollVelocity = derived(
  scrollTracker,
...
export const scrollProgress = derived(
  scrollTracker,
  ($position) => $position.normalizedProgress
);
...
export const sectionTransition = derived(
  scrollState,
  ($state) => ({
